    ...
    <div className="flex items-center justify-between border-b-2 border-brand-700 pb-3">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          {lab.address && <p className="text-[11px] text-[#475467]">{lab.address}</p>}
          <p className="text-[11px] text-[#475467]">
            {[lab.phone, lab.email, lab.website].filter(Boolean).join("  ·  ")}
          </p>
        </div>
      </div>
      {lab.panVat && <p className="text-[11px] text-[#475467]">PAN/VAT: {lab.panVat}</p>}
    </div>
  );
}
...
export function PrintFooter({ footerUrl, lab }: { footerUrl?: string | null; lab: LabInfo }) {
  ...
  return (
    <div className="border-t border-[#DFE2E2] pt-2 text-center text-[10px] text-[#647067]">
      {lab.name} · Generated by Nidanyo — From Liquid to Logic
    </div>
  );
}
